...
GAP_THRESHOLD = 5.0
TEST_MODE = True # Dushanba kuni False qilib qo'yasiz
    ...
    for i in range(0, len(TICKERS), chunk_size):
        ...
        try:
            ...
            for ticker in chunk:
                ...
                t_close = d_close[ticker].dropna()
                t_open = d_open[ticker].dropna()
                
                if len(t_close) < 2: continue
                
                if TEST_MODE:
                    p_close = float(t_close.iloc[-2])
                    c_price = float(t_open.iloc[-1])
                else:
                    dates_d = pd.to_datetime(t_close.index).tz_convert(ny_tz).date if t_close.index.tz is not None else pd.to_datetime(t_close.index).date
                    past_closes = t_close[dates_d < today_ny]
                    if len(past_closes) == 0: continue
                    p_close = float(past_closes.iloc[-1])
                    c_price = p_close
                    
                    if now_ny.time() >= market_open_time:
                        today_opens = t_open[dates_d == today_ny]
                        if len(today_opens) > 0:
                            c_price = float(today_opens.iloc[0])
                        else:
                            if ticker in m_close.columns:
                                t_m = m_close[ticker].dropna()
                                idx_ny = pd.to_datetime(t_m.index).tz_convert(ny_tz) if t_m.index.tz is not None else pd.to_datetime(t_m.index)
                                reg_session = t_m[idx_ny.time >= market_open_time]
        ...
        try:
            ...
            if finnhub_client:
                ...
                for article in reversed(news):
                    ...
                    if not TEST_MODE and ny_time < cutoff_time: continue
